...
prefix=dist
TARGET_LOADABLE=$(prefix)/debug/pdf0.$(LOADABLE_EXTENSION)
TARGET_LOADABLE_RELEASE=$(prefix)/release/pdf0.$(LOADABLE_EXTENSION)

TARGET_STATIC=$(prefix)/debug/pdf0.a
TARGET_STATIC_RELEASE=$(prefix)/release/pdf0.a

TARGET_WHEELS=$(prefix)/debug/wheels
TARGET_WHEELS_RELEASE=$(prefix)/release/wheels

INTERMEDIATE_PYPACKAGE_EXTENSION=python/sqlite_pdf/sqlite_pdf/pdf0.$(LOADABLE_EXTENSION)

ifdef target
CARGO_TARGET=--target=$(target)
BUILT_LOCATION=target/$(target)/debug/$(LIBRARY_PREFIX)sqlite_pdf.$(LOADABLE_EXTENSION)
BUILT_LOCATION_RELEASE=target/$(target)/release/$(LIBRARY_PREFIX)sqlite_pdf.$(LOADABLE_EXTENSION)
else
CARGO_TARGET=
BUILT_LOCATION=target/debug/$(LIBRARY_PREFIX)sqlite_pdf.$(LOADABLE_EXTENSION)
BUILT_LOCATION_RELEASE=target/release/$(LIBRARY_PREFIX)sqlite_pdf.$(LOADABLE_EXTENSION)
endif
...
python: $(TARGET_WHEELS) $(TARGET_LOADABLE) python/sqlite_pdf/setup.py python/sqlite_pdf/sqlite_pdf/__init__.py .github/workflows/rename-wheels.py
	...
	rm $(TARGET_WHEELS)/sqlite_pdf* || true
	pip3 wheel python/sqlite_pdf/ -w $(TARGET_WHEELS)
	python3 .github/workflows/rename-wheels.py $(TARGET_WHEELS) $(RENAME_WHEELS_ARGS)

python-release: $(TARGET_LOADABLE_RELEASE) $(TARGET_WHEELS_RELEASE) python/sqlite_pdf/setup.py python/sqlite_pdf/sqlite_pdf/__init__.py .github/workflows/rename-wheels.py
	cp $(TARGET_LOADABLE_RELEASE)  $(INTERMEDIATE_PYPACKAGE_EXTENSION)
	rm $(TARGET_WHEELS_RELEASE)/sqlite_pdf* || true
	pip3 wheel python/sqlite_pdf/ -w $(TARGET_WHEELS_RELEASE)
	python3 .github/workflows/rename-wheels.py $(TARGET_WHEELS_RELEASE) $(RENAME_WHEELS_ARGS)

npm: VERSION npm/platform-package.README.md.tmpl npm/platform-package.package.json.tmpl npm/sqlite-pdf/package.json.tmpl scripts/npm_generate_platform_packages.sh
...
python/sqlite_pdf/sqlite_pdf/version.py: VERSION
...
version: VERSION
	make Cargo.toml
	make python/sqlite_pdf/sqlite_pdf/version.py
	make npm
	make deno
	make ruby
